    ...
    for (ii = 0; ii < inum; ii++) {
        ...
        for (jj = 0; jj < jnum; jj++) {
            ...
            if (rsq < cutforcesq) {
                ...
                g2_v2gmap_tensor_mapped(nij, 1) = g2_offset_map[IJ(itype, jtype, ntypes)];
                nij += 1;
                if (i < inum && j < inum) {
                    ...
                    g2_v2gmap_tensor_mapped(nij, 1) = g2_offset_map[IJ(jtype, itype, ntypes)];
                    nij += 1;
                }
            }
        }
