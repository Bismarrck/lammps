...
VirtualAtomMap::VirtualAtomMap(Memory *pool, int n_elements_)
{
    n_atoms_vap = 0;
        ...
        const int *itypes)
{
    ...
    memory->create(local_to_vap_map, nlocal, "pair:vap:local2vap");
...
bool VirtualAtomMap::build_max_occurs(int nlocal, const int* atom_types)
{
    ...
    if (max_occurs == nullptr) {
        memory->create(max_occurs, size, "pair:vap:max_occurs");
    }

    if (is_changed) {
        curr_nlocal = nlocal;
        curr_itypes = memory->grow(curr_itypes, nlocal, "pair:vap:itypes");
        for (int i = 0; i < nlocal; i ++) {
